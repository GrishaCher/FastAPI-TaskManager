...
from app.services.CacheService import cache
...
async def read_user_tasks(
    ...
    tasks = await get_user_tasks(
        session=db, user_id=current_user.id, skip=skip, limit=limit, completed=completed
    )
    if tasks is None:
        raise HTTPException(status_code=404, detail="Tasks not found")
    if tasks.tasks != []:
        logger.info(
            f"Получены задачи {tasks.tasks[0]} - {tasks.tasks[len(tasks.tasks)-1]} пользователя с id: {current_user.id}"
        )
    else:
...
async def update_user_task(
    ...
    task = await update_task(
        session=db, user_id=current_user.id, task_id=task_id, task_in=task_data
    )
    if not task:
        raise HTTPException(status_code=400, detail="Bad Request")
    return TaskResponse.model_validate(task)
...
async def delete_user_task(
    ...
    delete_message = await delete_task_by_id(
        session=db, user_id=current_user.id, task_id=task_id
    )
