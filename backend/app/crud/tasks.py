...
async def create_task(
    *, session: AsyncSession, user_id: int, task_data: TaskCreate
    ...
    logger.debug(
        f"попытка создать задачу {task_data.title} пользователем с id: {user_id}"
    )
    db_task = Task(
...
async def update_task(
    *, session: AsyncSession, user_id: int, task_id: int, task_in: TaskInUpdate
) -> Task:
...
async def get_user_tasks(
    *, session: AsyncSession, user_id: int, skip, limit, completed
