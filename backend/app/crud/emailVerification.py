...
async def create_verification(*, session: AsyncSession, user_data: UserRegister) -> EmailVerificationDB:
...
async def get_verification(*, session: AsyncSession, token: str) -> EmailVerificationDB | None:
    result = await session.execute(
        select(EmailVerificationDB)
        .where(EmailVerificationDB.token == token)
    )
